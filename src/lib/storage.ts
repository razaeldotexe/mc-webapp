...
export function deleteFile(filePath: string): void {
  ...
  // Handle API-served thumbnail paths: /api/thumbnails/xxx -> /uploads/thumbnails/xxx
  let resolvedPath = filePath;
  if (filePath.startsWith("/api/thumbnails/")) {
    resolvedPath = filePath.replace("/api/thumbnails/", "/uploads/thumbnails/");
  }
  const fullPath = path.join(process.cwd(), "public", resolvedPath);
